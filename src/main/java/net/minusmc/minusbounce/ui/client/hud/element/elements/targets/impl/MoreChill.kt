...
class MoreChill(inst: Target) : TargetStyle("MoreChill", inst, true) {

    private val morechillFontSpeed = FloatValue("FontSpeed", 0.5F, 0.01F, 1F)
    private val morechillRoundValue = BoolValue("RoundedBar", true)
    ...
    override fun drawTarget(entity: EntityPlayer) {
        ...
        border.draw()
        // background
        RenderUtils.drawRoundedRect(0F, 0F, tWidth, 48F, 10F, targetInstance.bgColor.rgb)
        ...
        numberRenderer.renderChar(
            ...
            calcScaleY,
            false,
            morechillFontSpeed.get(),
            getColor(-1).rgb
        )
        // health bar
        RenderUtils.drawRoundedRect(4F, 38F, tWidth - 4F, 44F, 3F, targetInstance.barColor.rgb)
        Stencil.write(false)
        GL11.glDisable(GL11.GL_TEXTURE_2D)
        GL11.glEnable(GL11.GL_BLEND)
        GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA)
        ...
        GL11.glDisable(GL11.GL_BLEND)
        Stencil.erase(true)
        if (morechillRoundValue.get())
            RenderUtils.customRounded(4F, 38F, 4F + (easingHealth / entity.maxHealth) * (tWidth - 8F), 44F, 0F, 3F, 3F, 0F, Color(0xF50046).rgb) // Health bar color (F50046)
        else
            RenderUtils.drawRect(4F, 38F, 4F + (easingHealth / entity.maxHealth) * (tWidth - 8F), 44F, Color(0xF50046).rgb) // Health bar color (F50046)
        Stencil.dispose()
        Stencil.write(false)
        GL11.glDisable(GL11.GL_TEXTURE_2D)
        GL11.glEnable(GL11.GL_BLEND)
    ...
    override fun getBorder(entity: EntityPlayer?): Border {
        ...
        val tWidth = (45F + Fonts.font40.getStringWidth(entity.name).coerceAtLeast(Fonts.font72.getStringWidth(decimalFormat.format(entity.health)))).coerceAtLeast(120F)
        val borderColor = Color(0xF50046) // Border color (F50046)

        return Border(0F, 0F, tWidth, 48F, borderColor.rgb)
    }
}
